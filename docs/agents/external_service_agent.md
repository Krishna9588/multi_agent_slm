# External Service Agent (external_service_agent.py)

## Brief Description
Calls external cloud scraping platforms (Apify, Bright Data) to extract data from

## Prerequisites
1. **API Keys**: Keys for whatever external service you are attempting to contact.

## Step-by-Step Setup Guide
1. Identify the third-party service the agent needs to hit.
2. Export the appropriate token to your terminal environment before running the swarm.

## How to Update
- The code for this agent lives in `agents/external_service_agent.py`.
